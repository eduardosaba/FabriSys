...
export default function PurchaseOrdersWidget({
  ...
  const getStatusStyle = (status: string, date?: string) => {
    ...
    if (status === 'aprovado')
      return { color: 'text-blue-600 bg-blue-50 border-blue-200', icon: Truck, label: 'Aprovado' };
    return {
      ...
      <div className="flex flex-col h-full">
        {pedidos.length > 0 ? (
          <div className="flex-1 space-y-2 overflow-y-auto pr-1 custom-scrollbar">
            {pedidos.map((pedido) => {
              ...
              return (
                <div
                  key={pedido.id}
                  className="flex justify-between items-center p-2 rounded-lg border border-slate-50 hover:bg-slate-50 transition-colors group"
                >
                  <div className="min-w-0 flex-1 mr-2">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span
                        className={`text-[9px] px-1.5 py-0.5 rounded-full border uppercase font-bold flex items-center gap-1 ${style.color}`}
                      >
                  ...
                  <div className="text-right">
                    ...
                    {pedido.data_prevista && (
                      <span className="text-[10px] text-slate-400">
                        {format(parseISO(pedido.data_prevista), 'dd/MM', { locale: ptBR })}
                      </span>
                    )}
                  </div>
                </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center text-slate-400">
            ...
            <button
              onClick={() => router.push('/dashboard/insumos/pedidos-compra')}
