...
export default function LowStockWidget() {
  ...
  const fetchLowStock = useCallback(async () => {
    try {
      ...
      const { data: insumos, error } = await safeSelect(supabase, 'insumos', 'id, nome, estoque_atual, estoque_minimo_alerta, unidade_medida', (b: any) => b.limit(50));
      ...
      const criticos = (insumos || []).filter((item: any) =>
        (item.estoque_atual || 0) < (item.estoque_minimo_alerta || 0)
      );
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', 
        { event: '*', schema: 'public', table: 'insumos', filter: profile?.organization_id ? `organization_id=eq.${profile.organization_id}` : undefined },
        () => void fetchLowStock()
      )
      .subscribe();

    return () => { supabase.removeChannel(channel); };
  }, [fetchLowStock, profile?.organization_id]);
      ...
      <div className="flex flex-col h-full">
        {items.length > 0 ? (
          <div className="flex-1 space-y-3 overflow-y-auto pr-1 custom-scrollbar">
            {items.map((item) => {
              const percentual = item.estoque_minimo_alerta > 0 
                ...
                <div key={item.id} className="group">
                  <div className="flex justify-between items-center mb-1">
                    <div className="flex items-center gap-2 overflow-hidden">
                      ...
                      <span className="text-sm font-medium text-slate-700 truncate" title={item.nome}>
                        {item.nome}
                      </span>
                    </div>
                    <span className={`text-xs font-bold ${isZerado ? 'text-red-600' : 'text-slate-600'}`}>
                      {item.estoque_atual} <span className="text-[10px] font-normal text-slate-400">{item.unidade_medida || 'un'}</span>
                    </span>
                  </div>
