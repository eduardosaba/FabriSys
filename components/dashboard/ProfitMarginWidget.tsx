    ...
    async function calculateProfit() {
      ...
      try {
        ...
        const faturamentoTotal = vendas.reduce(
          (acc: number, v: any) => acc + Number(v.total_venda || 0),
          0
        );
        ...
        <div className="flex-1 flex flex-col justify-between py-2">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span
                className={`p-1.5 rounded-md text-white ${isHealthy ? 'bg-emerald-500' : isCritical ? 'bg-red-500' : 'bg-yellow-500'}`}
              >
            ...
            <h3
              className={`text-4xl font-bold tracking-tighter ${isHealthy ? 'text-emerald-600' : isCritical ? 'text-red-600' : 'text-yellow-600'}`}
            >
              {metrics.margemPercentual.toFixed(1)}%
            </h3>
            <p className="text-[10px] text-slate-400 mt-2 leading-tight">
              Isso significa que para cada R$ 100 vendidos, sobram{' '}
              <strong>R$ {metrics.margemPercentual.toFixed(0)}</strong> para pagar despesas fixas.
            </p>
          </div>
