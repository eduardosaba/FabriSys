...
const MODULOS = [
  ...
  { id: 'fabrica_dashboard', label: 'Dashboard Fábrica' },
  ...
  { id: 'estoque_fabrica', label: 'Estoque Fábrica' },
  ...
  { id: 'pdv_inventario', label: 'Inventário de Loja' },

  // Logística
  { id: 'logistica', label: 'Logística (menu)' },
  { id: 'logistica_expedicao', label: 'Expedição' },

  // Suprimentos / Insumos
  { id: 'suprimentos', label: 'Suprimentos & Compras' },
  { id: 'compras_sugestao', label: 'Sugestão de Compras (MRP)' },
  { id: 'pedidos_compra', label: 'Pedidos de Compra' },
  { id: 'insumos_estoque', label: 'Estoque de Insumos' },
  { id: 'insumos_alertas', label: 'Monitor de Riscos' },
  { id: 'insumos_cadastro', label: 'Cadastro de Insumos' },
  { id: 'insumos_categorias', label: 'Categorias de Insumos' },
  ...
  { id: 'relatorios_painel', label: 'Relatórios: Painel Gerencial' },
  { id: 'relatorios_vendas', label: 'Relatórios: Vendas Consolidadas' },
  { id: 'relatorios_historico_caixa', label: 'Relatórios: Histórico de Fechamentos' },
  { id: 'relatorios_estoque', label: 'Relatórios: Posição de Estoque' },
  { id: 'relatorios_validade', label: 'Relatórios: Validade & Perdas' },
  ...
  { id: 'financeiro_dre', label: 'DRE - Financeiro' },
  { id: 'financeiro_categorias', label: 'Categorias Financeiras' },
  ...
  // Admin
  { id: 'admin', label: 'Admin Master (menu)' },
  { id: 'admin_novo_cliente', label: 'Admin: Criar Cliente' },
  { id: 'admin_usuarios', label: 'Admin: Equipe & Usuários' },
...
export default function PermissoesTab() {
  ...
  const perfisVisiveis = useMemo(() => {
    if (profile?.role === 'admin') {
      return PERFIS.filter((p) => p.id !== 'master');
    }
    return PERFIS;
  }, [profile?.role]);
  ...
  const modulosVisiveis = useMemo(() => {
    if (profile?.role === 'admin') {
      return MODULOS.filter((m) => !String(m.id).startsWith('admin'));
    }
    return MODULOS;
  }, [profile?.role]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const savePromise = (async () => {
        ...
        const { data: _verifyData, error: verifyErr } = await verifyQuery;
  ...
  return (
    <div className="space-y-6 animate-fade-up pb-20 md:pb-0">
      <div className="bg-primary border-primary rounded-xl p-4 flex gap-3 items-start">
        ...
        <div>
          <h4 className="font-bold text-white text-sm">Controle de Segurança</h4>
      ...
      <div className="mt-4">
        <nav className="flex flex-wrap items-end gap-2 mb-4 border-b border-slate-100">
          <button
          ...
          <div className="hidden md:block ml-auto text-xs text-slate-400 px-2 py-2">
