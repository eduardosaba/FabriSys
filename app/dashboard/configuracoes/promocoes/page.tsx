...
export default function PromocoesPage() {
  ...
  const totalOriginal = itensCombo.reduce((acc, i) => acc + (i.quantidade * i.produto.preco_venda), 0);
  ...
  return (
    <div className="p-6 animate-fade-up">
      <PageHeader 
        ...
        icon={Tag}
      >
        <Button icon={Plus} onClick={openNew}>Nova Promoção</Button>
      </PageHeader>

      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {promocoes.map(promo => (
          <div key={promo.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group hover:shadow-md transition-all">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-bold text-slate-800 text-lg">{promo.nome}</h3>
                <p className="text-sm text-slate-500">{promo.itens?.length || 0} produtos inclusos</p>
              </div>
              <div className="text-right">
                <span className="block font-bold text-green-600 text-xl">
                  {formatCurrency(promo.preco_total)}
                </span>
                <span className={`text-xs px-2 py-0.5 rounded-full ${promo.ativo ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Editar Promoção' : 'Criar Novo Combo'}>
        <div className="p-4 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Preço Final (Venda)</label>
                <CurrencyInput
                ...
                placeholder="0,00"
                value={precoFinal}
                onChange={(v) => setPrecoFinal(typeof v === 'string' ? v : (v.target as HTMLInputElement).value)}
              />
            </div>

            <div className="flex flex-col justify-end pb-2">
               <span className="text-xs text-slate-500">Valor Original dos Itens:</span>
               <span className="text-sm font-medium line-through text-slate-400">{formatCurrency(totalOriginal)}</span>
               {desconto > 0 && <span className="text-xs text-green-600 font-bold">Desconto: {formatCurrency(desconto)}</span>}
            </div>
          </div>
          ...
          {/* Adicionar Itens */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Composição do Combo</label>
            <div className="flex gap-2 mb-4">
              ...
              <Button size="sm" onClick={handleAddItem} disabled={!selectedProdId} icon={Plus}>Adicionar</Button>
            </div>

            {/* Lista de Itens no Combo */}
            <div className="bg-slate-50 rounded-lg border border-slate-200 max-h-60 overflow-y-auto">
              {itensCombo.length === 0 ? (
                <p className="text-center text-slate-400 py-4 text-sm">Adicione produtos para compor o combo.</p>
              ) : (
                itensCombo.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-2 border-b last:border-0">
                    <div className="flex-1">
                      <span className="text-sm font-bold text-slate-700 block">{item.produto.nome}</span>
                      <span className="text-xs text-slate-400">Preço orig: {formatCurrency(item.produto.preco_venda)}</span>
                    </div>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <div className="flex items-center gap-1">
                        ...
                        <input 
                          ...
                          onChange={e => updateItem(item.produto.id, 'quantidade', Number(e.target.value))}
                        />
                      </div>
                      
                      <div className="flex items-center gap-1">
                         <label className="text-[10px] uppercase font-bold text-slate-400" title="Quanto este item custará DENTRO do combo (para relatórios)">Ref R$</label>
                         <input 
                            type="number" 
                            step="0.01"
                            className="w-16 text-center text-xs border rounded p-1 bg-white"
                            value={item.valor_referencia}
                            onChange={e => updateItem(item.produto.id, 'valor_referencia', Number(e.target.value))}
            ...
            <p className="text-[10px] text-slate-400 mt-1">* Ajuste o "Ref R$" para distribuir o desconto corretamente nos relatórios.</p>
          </div>

          <div className="flex justify-end gap-2 pt-4 border-t">
            <Button variant="secondary" onClick={() => setIsModalOpen(false)}>Cancelar</Button>
            <Button onClick={handleSave} icon={Save}>Salvar Promoção</Button>
          </div>
        </div>
      </Modal>
    </div>
