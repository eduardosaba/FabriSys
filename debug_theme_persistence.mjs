...
(async () => {
  try {
    ...
    if (!user) {
      console.warn('⚠️  Nenhum usuário autenticado. Listando todos os registros em user_theme_colors...\n');
    } else {
    ...
    if (user) {
      ...
      if (userError) {
        ...
        userRows.forEach((row) => {
          console.log(`[${row.theme_mode.toUpperCase()}] ${row.primary_color} | ${row.sidebar_bg} | ${new Date(row.updated_at).toLocaleString('pt-BR')}`);
        });
      }
    }
