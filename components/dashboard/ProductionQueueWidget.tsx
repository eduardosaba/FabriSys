...
import { Hammer, Clock, AlertCircle, CheckCircle2, ArrowRight, CalendarDays } from 'lucide-react';
...
export default function ProductionQueueWidget({
  ...
  const fetchQueue = useCallback(async () => {
    try {
      ...
      const dateMap: Record<
        string,
        { data_entrega?: string | null; data_entrega_prevista?: string | null }
      > = {};
      try {
        const ordIds = Array.from(new Set(rows.map((r: any) => String(r.id)).filter(Boolean)));
        if (ordIds.length > 0) {
          const { data: datesData, error: datesErr } = await safeSelect(
            supabase,
            'ordens_producao',
          ...
          if (!datesErr && datesData) {
            (datesData || []).forEach((d: any) => {
              dateMap[String(d.id)] = {
                ...
                data_entrega_prevista: d.data_entrega_prevista ?? null,
              };
            });
          }
        }
      ...
      const pickDate = (r: any) =>
        ...
        dateMap[String(r.id)]?.data_entrega ||
        dateMap[String(r.id)]?.data_entrega_prevista ||
        null;
  ...
  return (
    <Card title="Fila de Produção" size="1x1" loading={loading} action={ActionButton}>
        ...
        {ordens.length > 0 ? (
          <div className="flex-1 space-y-3 overflow-y-auto pr-1 custom-scrollbar">
            {ordens.map((ordem) => (
              <div
                key={ordem.id}
                className="group flex items-center justify-between p-2 hover:bg-slate-50 rounded-lg border border-transparent hover:border-slate-100 transition-colors"
              >
                {/* Esquerda: Ícone + Infos */}
                <div className="flex items-center gap-3 overflow-hidden">
                  <div
                    className={`p-2 rounded-lg shrink-0 ${ordem.status === 'em_producao' ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-400'}`}
                  >
                    <Hammer size={16} />
                  </div>
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-slate-500 mt-0.5">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {(() => {
                          try {
                            return ordem.data_entrega
                              ? formatDistanceToNow(parseISO(String(ordem.data_entrega)), {
                                  addSuffix: true,
                                  locale: ptBR,
                                })
                              : 'Sem data';
