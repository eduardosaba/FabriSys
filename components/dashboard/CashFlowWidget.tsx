      ...
      try {
        ...
        try {
          const resp = await safeSelect(
            supabase,
            'vendas',
            'total_venda,forma_pagamento,caixa_validado,created_at',
            (b: any) =>
              b
                ...
                .or('forma_pagamento.neq.dinheiro,caixa_validado.eq.true')
          );
          ...
          if (/created_at/i.test(msg) || /column\s+.*created_at.*does not exist/i.test(msg)) {
            const fallback = await safeSelect(
              ...
              'vendas',
              'total_venda,forma_pagamento,caixa_validado,created_at',
              (b: any) => b.eq('organization_id', organizationId).eq('status', 'concluida')
            );
        ...
        const totalEntradas =
          vendas?.reduce((acc: number, v: any) => acc + Number(v.total_venda || 0), 0) || 0;
        const totalSaidas =
          despesas?.reduce((acc: number, d: any) => acc + Number(d.valor_total || 0), 0) || 0;
        ...
        <div className="flex-1 flex flex-col justify-center space-y-4">
          ...
          <div className="pt-2 border-t border-slate-100">
            <div className="flex justify-between items-end">
              ...
              <span
                className={`text-xl font-black ${resumo.saldo >= 0 ? 'text-blue-600' : 'text-red-600'}`}
              >
                {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(
                  resumo.saldo
                )}
