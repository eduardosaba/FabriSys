...
export default function ValorProducaoWidget() {
  ...
  const [valorProducao, setValorProducao] = useState(0);
    ...
    const [periodo, setPeriodo] = useState<'mes-atual'|'ultimos-30-dias'|'personalizado'>('mes-atual');
      ...
      try {
        ...
        if (produtoIds.length > 0) {
          ...
          for (let i = 0; i < produtoIds.length; i += chunkSize) {
            ...
            (produtos || []).forEach((p: any) => (produtoMap[String(p.id)] = { preco_venda: Number(p.preco_venda || 0) }));
          }
        }
        ...
        if (!metasErr) {
          const metaTotal = (metasData || []).reduce((s: number, m: any) => s + Number(m.valor_meta || 0), 0);
    ...
    return (
      <div className="w-full">
        <div className="mb-3 flex items-center justify-end gap-2">
          <select value={periodo} onChange={(e) => setPeriodo(e.target.value as any)} className="text-sm border rounded px-2 py-1 bg-white">
          ...
          {periodo === 'personalizado' && (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => void loadKPIs()} className="text-sm bg-indigo-600 text-white px-3 py-1 rounded">Aplicar</button>
            </div>
          )}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
            ...
            <p className="text-sm font-medium text-slate-500 flex items-center gap-2">Valor Produção</p>
            <h3 className="text-2xl font-bold text-slate-800 mt-2">{loading ? '...' : valorProducao.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</h3>
          </div>
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
            ...
            <h3 className="text-2xl font-bold text-slate-800 mt-2">{loading ? '...' : metaMensal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</h3>
            <p className="text-xs text-slate-500 mt-1">Faltam: <span className="font-bold text-red-500">{loading ? '...' : metaFaltante.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></p>
          </div>

          <div onClick={() => router.push('/dashboard/producao/kanban')} className="bg-blue-50 p-4 rounded-xl border border-blue-100 shadow-sm cursor-pointer relative overflow-hidden group hover:shadow-md transition-all">
          ...
          <div onClick={() => router.push('/dashboard/insumos/alertas')} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm cursor-pointer relative overflow-hidden group hover:shadow-md transition-all">
            <div className="absolute right-0 top-0 p-4 opacity-10">
              <AlertTriangle size={48} className={estoqueCritico > 0 ? 'text-red-600' : 'text-slate-400'} />
            </div>
            <p className="text-sm font-medium text-slate-500 flex items-center gap-2">Estoque Crítico</p>
            <h3 className={`text-3xl font-bold mt-2 ${estoqueCritico > 0 ? 'text-red-600' : 'text-slate-800'}`}>{loading ? '...' : estoqueCritico}</h3>
          </div>
        </div>
      </div>
    );
