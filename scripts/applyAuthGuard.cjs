...
function applyGuardToFile(filePath) {
  ...
  if (!/useAuth/.test(src)) {
    ...
    if (importMatch) {
      ...
      src = src.slice(0, insertAt) + "import { useAuth } from '@/lib/auth';\n" + src.slice(insertAt);
      modified = true;
    } else {
      // fallback: add at top
  ...
  src = src.replace(/useEffect\s*\((\s*function\s*\w*\s*\(|\s*\(\)\s*=>\s*\(|\s*async\s*function\s*\(|\s*async\s*\(\)\s*=>\s*\(){([\s\S]*?)^\s*\}\s*,/mg, (match, p1, body) => {
    if (!/supabase\./.test(body)) return match;
    if (/if\s*\(authLoading\)/.test(body) || /if\s*\(!profile\?\./.test(body)) return match;
    const injected = match.replace(/useEffect\s*\(([\s\S]*?\(){/, (m) => m + '\n    if (authLoading) return;\n    if (!profile?.organization_id) return;');
    modified = true;
    return injected;
  });
