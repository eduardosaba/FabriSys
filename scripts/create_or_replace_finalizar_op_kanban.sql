-- Atualiza/Cria a função finalizar_op_kanban para garantir que a produção aumente o estoque da fábrica
...
BEGIN
    ...
    -- 3. VALIDAR SE ENCONTROU FÁBRICA
    IF v_local_fabrica_id IS NULL THEN
        RETURN json_build_object('success', false, 'message', 'Fábrica não encontrada para esta organização');
    END IF;

    -- 4. ATUALIZAR ESTOQUE DA FÁBRICA
    ...
    RETURN json_build_object('success', true, 'message', 'Produção finalizada e estoque abastecido');
END;
$function$;

-- Execute este script no SQL Editor do Supabase/pgAdmin/psql para garantir que a RPC funcione corretamente.
