...
export default function ProfilePage() {
  ...
  const fetchAvatarLibrary = async () => {
    ...
    try {
      const { data, error } = await supabase.storage
        .from('avatars')
        .list('biblio_avatar', { limit: 200, offset: 0, sortBy: { column: 'name', order: 'asc' } });
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      try {
        toast({ title: 'Erro', description: String(err.message || err), variant: 'error' });
      } catch (e) {
        void e;
      }
    }
  ...
  const handleSelectLibraryAvatar = async (url: string) => {
    ...
    try {
      ...
      try {
        toast({ title: 'Avatar selecionado' });
      } catch (e) {
        void e;
      }
    } catch (e) {
  ...
  const handleChangePassword = async () => {
    if (passData.newPassword !== passData.confirmPassword)
      return tryToastError('As senhas não coincidem!');
    if (passData.newPassword.length < 6)
      return tryToastError('A senha deve ter no mínimo 6 caracteres.');
    setSaving(true);
    try {
  ...
  const tryToastError = (msg: string) => {
    try {
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className={`${cardClass} flex flex-col items-center text-center`}>
            ...
            <div className="w-full border-t border-gray-100 dark:border-slate-800 pt-4 mt-2">
              <span
                className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${planData.name === 'Gratuito' ? 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-300' : 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400'}`}
              >
          ...
          {activeTab === 'profile' && (
            <section className={cardClass}>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-6 flex items-center gap-2 border-b border-gray-100 dark:border-slate-800 pb-4">
                <User size={20} className="text-[var(--primary)]" /> Editar Perfil
              </h3>
                ...
                <div className="col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData((p: any) => ({ ...p, full_name: e.target.value }))
                      }
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData((p: any) => ({ ...p, store_name: e.target.value }))
                      }
              ...
              <div className="mt-8 flex justify-end pt-4 border-t border-gray-100 dark:border-slate-800">
                <Button
                  ...
                  icon={Save}
                  variant="primary"
                  className="font-bold px-6 py-2.5 rounded-lg shadow-md"
                >
                  Salvar Dados
                </Button>
              </div>
                ...
                <div className="flex justify-between items-start relative z-10">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${planData.status === 'active' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 border-red-200 dark:border-red-800'}`}
                      >
                        <span
                          className={`w-2 h-2 rounded-full ${planData.status === 'active' ? 'bg-green-500' : 'bg-red-500'}`}
                        ></span>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[80vh] border border-gray-200 dark:border-slate-800">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-900">
              <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <Smile size={20} className="text-[var(--primary)]" /> Galeria de Avatares
              </h3>
              <button
