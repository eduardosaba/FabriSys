    ...
    <Card title="Status do Meu Caixa" size="1x1" loading={loading}>
      <div className="flex flex-col h-full justify-between">
        <div className="flex items-center gap-4">
          <div
            className={`p-4 rounded-full ${sessao ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-500'}`}
          >
          ...
          <div>
            ...
            {sessao ? (
              <p className="text-xs text-slate-500">
                ...
                {(() => {
                  try {
                    return sessao?.data_abertura
                      ? formatDistanceToNow(parseISO(String(sessao.data_abertura)), {
                          addSuffix: true,
                          locale: ptBR,
                        })
                      : '';
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {sessao ? (
            <>
              <button
                onClick={irParaVenda}
                className="col-span-2 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-bold transition-colors shadow-sm"
              >
              ...
              <button
                onClick={fecharCaixa}
                className="flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 py-2 rounded-lg text-xs font-medium transition-colors"
              >
            ...
            <button
              onClick={abrirCaixa}
              className="col-span-2 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-lg font-bold transition-colors shadow-sm animate-pulse"
            >
